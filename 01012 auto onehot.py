...
def my_loss(x,y):
    ...
    std = K.std((x-y),-1)
    mean = K.mean((x-y),-1)
    return bc + abs(mean)#+abs(std-0.2)
...
def model_loss(x, y):
    bc = binary_crossentropy(x, y)
    #std = K.std(models['n'].layers[-1].output, -1)
    #mean = K.mean(models['n'].layers[-1].output, -1)
    return bc +K.mean(models['a'].layers[-2].layers[-1].output)

models = make_model()
optim = optims.Adam(learning_rate=0.0001)
models['n'].trainable = False
#models['e'].trainable = False

models['a'].compile(optimizer=optim,
              #loss='categorical_crossentropy',#bit_mean_abs,#'mae',
              #loss = models['l'](models['d'].outputs[0]),
...
def enc_dec_train_get(cnt):
  ...
  #x = [[] for _ in range(9*3)]
  #y = []
  #mem = None
  #x = np.zeros(shape=(cnt, 9*3, size))
  x = np.random.randint(2**bits_per_input,size=(cnt+3)*9)
  x = coder.encode(x)
  #max = 0
  #for i in range(cnt):
  #    for j in range(9*3):
  ...
  #            max = k
  #        x[i,j,k] = 1.0
  #xx = [[x[k] for k in range(i,i+3*9)]+[x[k] for k in range(i+3*9,i+2*3*9)] for i in range(0,cnt*9*3,9*3)]
  xx = [x[i:-27+i:9] for i in range(9*3)]
  y = x[4:-9*3+4:9]
  return (xx,y)
x,y = enc_dec_train_get(1024*8)
print("t1")
...
if not graph:
 while True:
  models['a'].fit(x,y,batch_size=batch,epochs=2000,
