...
class Encoder(nn.Module):
    ...
    def forward(self,inp):
        #inp = [samples, back_samples]
        x = inp[0].view(-1,self._width,1,1)
        #y = inp[1].view(-1, self._width, 1, 1)
...
class Decoder(nn.Module):
    ...
    def forward(self, inp):
        #inp = [front_pixels, diff_back_pixels_reencoded]
...
if __name__=='__main__':
    ...
    ff_write_file = 'ffmpeg -y -loglevel panic -f rawvideo -pix_fmt gray -s:v {0}x{1} -r 60 -i - -c:v libx264 -preset fast -crf 26 testt.mp4'
