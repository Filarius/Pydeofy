...
bad_luck_row = 0
last_luck_row = 0
#f1 = open('text1.txt','wb')
#f2 = open('text2.txt','wb')
for x in frame_samples_gen(frame_cnt):
    ...
    q = K.eval(K.mean(binary_accuracy(y, x[0])))
    #y1 = lcoder.inverse_transform(y)
    #y2 = lcoder.inverse_transform(x[0])
    y1 = coder.decode(y)
    y2 = coder.decode(x[0])
    ...
    rets = 0
    for i in range(samples_per_frame):
        #x0 = np.argmax(x[0][i]).astype(np.uint16)
        #x1 = np.argmax(y[i]).astype(np.uint16)
        x0 = y1[i]
        x1 = y2[i]
        #f1.write(x0)
        #f2.write(x1)
    ...
    #print(cnt / samples_per_frame, rets / (bads+1), bad_luck_row)
    print(q, cnt / samples_per_frame, bad_luck_row)
